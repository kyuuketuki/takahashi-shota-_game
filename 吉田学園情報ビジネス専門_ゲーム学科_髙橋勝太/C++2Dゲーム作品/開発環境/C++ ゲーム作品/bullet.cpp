...
HRESULT CBullet::Init(D3DXVECTOR3 pos, D3DXVECTOR3 move, D3DXCOLOR col, float fSpeed, int nLife, int nDamage, float fSizeX, float fSizeY, int nType, int nTypeMove)
{
	...
	//敵からの発射された弾の場合
	if (nType == OBJTYPE_BULET_ENEMY || nType == OBJTYPE_BULET_ENEMY2 || nType == OBJTYPE_BULET_ENEMY3)
	{
		// オブジェクトの種類設定
...
void CBullet::Update(void)
{
	...
	if (g_TypeMoveAction == m_TypeMove)
	{
		...
		if (g_Action == 2)
		{
			...
			// 目標までの距離dを求める
			float d = sqrt((g_nextpos.x - m_oldpos.x)*(g_nextpos.x - m_oldpos.x) + (g_nextpos.y - m_oldpos.y)*(g_nextpos.y - m_oldpos.y));

			float vx;
			float vy;

			// 速さが一定値speedになるように速度を求める
			vx = (g_nextpos.x - m_oldpos.x) / d*g_fSpeed;
			vy = (g_nextpos.y - m_oldpos.y) / d*g_fSpeed;
	...
	else if (g_TypeMoveAction2 == m_TypeMove)
	{
		...
		if (g_Action2 == 2)
		{
			...
			// 目標までの距離dを求める
			float d = sqrt((g_nextpos2.x - m_oldpos.x)*(g_nextpos2.x - m_oldpos.x) + (g_nextpos2.y - m_oldpos.y)*(g_nextpos2.y - m_oldpos.y));

			float vx;
			float vy;

			// 速さが一定値speedになるように速度を求める
			vx = (g_nextpos2.x - m_oldpos.x) / d*g_fSpeed2;
			vy = (g_nextpos2.y - m_oldpos.y) / d*g_fSpeed2;
	...
	else if (g_TypeMoveAction3 == m_TypeMove)
	{
		...
		if (g_Action3 == 2)
		{
			...
			// 目標までの距離dを求める
			float d = sqrt((g_nextpos3.x - m_oldpos.x)*(g_nextpos3.x - m_oldpos.x) + (g_nextpos3.y - m_oldpos.y)*(g_nextpos3.y - m_oldpos.y));

			float vx;
			float vy;

			// 速さが一定値speedになるように速度を求める
			vx = (g_nextpos3.x - m_oldpos.x) / d*g_fSpeed3;
			vy = (g_nextpos3.y - m_oldpos.y) / d*g_fSpeed3;
...
{
	// 目標までの距離dを求める
	float d = sqrt((posPlayer.x - posBullet.x)*(posPlayer.x - posBullet.x) + (posPlayer.y - posBullet.y)*(posPlayer.y - posBullet.y));

	float vx;
	float vy;

	// 速さが一定値speedになるように速度を求める
	vx = (posPlayer.x - posBullet.x) / d*speed;
	vy = (posPlayer.y - posBullet.y) / d*speed;
