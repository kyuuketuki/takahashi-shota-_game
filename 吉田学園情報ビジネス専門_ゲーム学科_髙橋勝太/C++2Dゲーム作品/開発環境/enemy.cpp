...
//移動処理に使用
bool g_bUseEnemy = false;
bool g_bUseEnemy2 = false;
bool g_bUseEnemy3 = false;
		...
		if (m_nType == ENEMY_05)
		{
			...
			if (m_nAttackCnt2 >= 40)
			{
				for (int nEnemyBullet = 0; nEnemyBullet <= 20; nEnemyBullet++)
				{
					...
					int BulletRand = (D3DX_PI * 100) + (nCntBulletRand - 150);

					//弾の発射
					CBullet::InitAngleBullet(D3DXVECTOR3(pos.x, pos.y, 1.0f), -5.0f, D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f), 500, 15, BULLET_SIZE, BULLET_SIZE, BulletRand, OBJTYPE_BULET_ENEMY);
				}
	...
	if (m_bInvincibility == true)
	{
		...
		if (m_nHitTime == 1)
		{
			...
			for (int nCnt = 0; nCnt < 10; nCnt++)
			{
				...
				//パーティクル
				CParticl::Create(pos, D3DXCOLOR(1.0f, 0.0f, 0.0f, 0.5f), 20, nRandSpeed, 20, 20, nRand);
			}
		}
	}
