...
void DrawFPS(void)
{
	...
	nNum += sprintf(&aStr[nNum], "TOOL   pos:  X%.2f Y%.2f  Z%.2f\n\n", pCar->pos.x, pCar->pos.y, pCar->pos.z);
	...
	nNum += sprintf(&aStr[nNum], "1     :  ツール画面\n");
	nNum += sprintf(&aStr[nNum], "1     :  モデル画面\n\n");

	nNum += sprintf(&aStr[nNum], "-----TOOL-----\n");
	nNum += sprintf(&aStr[nNum], "8     :  視点の高さUP\n");
	nNum += sprintf(&aStr[nNum], "9     :  視点の高さDOWN\n");
	nNum += sprintf(&aStr[nNum], "G     :  オブジェクト生成 & data/ TEXT/Object.txtに保存\n");
	nNum += sprintf(&aStr[nNum], "H     :  オブジェクト消去 & data/ TEXT/Object.txtを修正\n");
	nNum += sprintf(&aStr[nNum], "T     :  オブジェクト生成 & data/ TEXT/Item.txtに保存\n");
	nNum += sprintf(&aStr[nNum], "Y     :  オブジェクト消去 & data/ TEXT/Item.txtを修正\n");
