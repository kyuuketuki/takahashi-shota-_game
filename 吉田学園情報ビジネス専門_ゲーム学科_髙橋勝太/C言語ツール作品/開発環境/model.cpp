...
void UpdatePlayer(void)
{
	...
	// モデルの移動
	if (GetKeyboardPress(DIK_W) == true || IsButtonPush(LEFT) || BuPu(LEFT, 0))
	{
	...
	else if (GetKeyboardPress(DIK_S) == true || BuPu(RIGHT, 1))
	{
		g_player.motionType = MOTIONTYPE_MOVE;
	...
	//移動キーを押したとき
	if (GetKeyboardTrigger(DIK_W) == true || GetKeyboardTrigger(DIK_S) == true || GetKeyboardTrigger(DIK_D) == true || GetKeyboardTrigger(DIK_A) == true || IsButtonDown(LEFT) || IsButtonDown(RIGHT))
	{
		g_player.nCounterMotion = 0;
	...
	//移動キーをはなしたとき
	if (GetKeyboardRelease(DIK_W) == true || GetKeyboardRelease(DIK_S) == true || GetKeyboardRelease(DIK_D) == true || GetKeyboardRelease(DIK_A) == true || IsButtonUp(LEFT) || IsButtonDown(RIGHT))
	{
		g_player.nCounterMotion = 0;
		g_player.nKey = 0;
		g_player.aModel[0].pos.y = 55.0f;
...
void SetMotion(MOTIONTYPE motionType)
{
	...
	//{// モーション
	//	g_player.bLoopMotion = true;					// ループしない
	//	g_player.nNumKey = 3;							// キー数
	//	g_player.aMotionInfo[1].aKeySet[0].nFrame = 20;	// フレーム数
	...
	//						 
	//	g_player.aMotionInfo[2].aKeySet[1].nFrame = 20;	// フレーム数
	//						 
	...
	//						 
	//						 
	//	g_player.aMotionInfo[2].aKeySet[2].nFrame = 40;	// フレーム数
	//						 
