...
void UpdateCamera(void)
{
	for (int nCnt = 0; nCnt < 2; nCnt++)
	{
		...
		if (GetGame() == 1)
		{
			...
			{
				g_camera[1].rot = D3DXVECTOR3(0.0f, 0.0f, 0.0f);	// カメラの角度
				nHeight2 = 0.0f;
			}
		}
		...
		//更新
		g_camera[1].fHeightV = nHeight2;						//視点の高さの更新
		g_camera[1].fLength = CAMERA_RANGE_MODEL;				//視点の距離の更新
...
void SetCamera(int nIdx)
{
	...
	D3DXMatrixPerspectiveFovLH(
		...
		(float)SCREEN_WIDTH / 2 / SCREEN_HEIGHT,				// 比率
		10.0f,													// 手前限界の描画距離
		4000.0f);												// 奥限界の描画距離
...
void CameraInfo(int nIdx)
{
	...
	case 1:
		...
		g_camera[nIdx].fLength = 500;							// 視点・注視点間の距離
		g_camera[nIdx].fHeightV = -60.0f;						// 視点の高さ
		g_camera[nIdx].fHeightR = 100.0f;		// 注視点の高さ
		...
		g_camera[nIdx].viewport.Width = (DWORD)960.0f;			// 描画する画面の幅
	...
	case 0:
		...
		g_camera[nIdx].fFront = 15.0f;							// モデルの前方の距離
		g_camera[nIdx].bAutoTurn = false;						// カメラの回り込みON・OFF
		g_camera[nIdx].viewport.X = (DWORD)960.0f;				// 描画する画面左上のＸ座標
		g_camera[nIdx].viewport.Y = (DWORD)0.0f;				// 描画する画面左上のＹ座標 
		g_camera[nIdx].viewport.Width = (DWORD)970.0f;			// 描画する画面の幅
		g_camera[nIdx].viewport.Height = (DWORD)1080.0f;		// 描画する画面の高さ
		g_camera[nIdx].viewport.MinZ = 0.0f;
		g_camera[nIdx].viewport.MaxZ = 1.0f;
