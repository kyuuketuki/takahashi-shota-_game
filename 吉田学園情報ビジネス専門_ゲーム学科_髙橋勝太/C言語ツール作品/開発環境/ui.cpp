...
#define PLAYER_BAR_Y	(250.0f)							// ロゴの高さ
...
HRESULT InitUi(void)
{
	...
	g_posPlayer = D3DXVECTOR3(SCREEN_WIDTH / 2, HEIGHT_Y, 0.0f);
	g_posPlayerBar = D3DXVECTOR3(400.0f, 970.0f, 0.0f);

	//テキスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/tool.png", &g_pTextureUi[1]);
	D3DXCreateTextureFromFile(pDevice, "data/TEXTURE/tool4.png", &g_pTextureUi[0]);
...
void SetVertexUi(int nCntUi)
{
	...
	if (nCntUi == 0)
	{
		pVtx[0].pos = D3DXVECTOR3(g_posPlayerBar.x - (PLAYER_BAR_X / 2), g_posPlayerBar.y + (PLAYER_BAR_Y / 2), 0.0f);
		pVtx[1].pos = D3DXVECTOR3(g_posPlayerBar.x - (PLAYER_BAR_X / 2), g_posPlayerBar.y - (PLAYER_BAR_Y / 2), 0.0f);
		pVtx[2].pos = D3DXVECTOR3(g_posPlayerBar.x + (PLAYER_BAR_X / 2) - (g_PlayerBar), g_posPlayerBar.y + (PLAYER_BAR_Y / 2), 0.0f);
		pVtx[3].pos = D3DXVECTOR3(g_posPlayerBar.x + (PLAYER_BAR_X / 2) - (g_PlayerBar), g_posPlayerBar.y - (PLAYER_BAR_Y / 2), 0.0f);
	}

	g_playerDate = GetGame();
	...
	if (nCntUi == 1)
	{
		...
		if (g_playerDate == 0)
		{
			g_posPlayerBar = D3DXVECTOR3(1500.0f, 970.0f, 0.0f);
			pVtx[4].pos = D3DXVECTOR3(0, SCREEN_HEIGHT, 0.0f);
			pVtx[5].pos = D3DXVECTOR3(0, 0, 0.0f);
			pVtx[6].pos = D3DXVECTOR3(SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f);
			pVtx[7].pos = D3DXVECTOR3(SCREEN_WIDTH, 0, 0.0f);
